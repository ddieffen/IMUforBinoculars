...
void Mpu9250::readAccelData(float * destination)
{
  ...
  destination[0] = (float)accelCount[0] * getAres();  // Turn the MSB and LSB into a signed 16-bit value
  destination[1] = (float)accelCount[1] * getAres();
  destination[2] = (float)accelCount[2] * getAres();
}
...
void Mpu9250::readGyroData(float * destination)
{
  ...
  destination[0] = (float)gyroCount[0] * getGres();  // Turn the MSB and LSB into a signed 16-bit value
  destination[1] = (float)gyroCount[1] * getGres();
  destination[2] = (float)gyroCount[2] * getGres();
}
...
void Mpu9250::Setup()
{
  ...
  while (c != 0x71) { //loop in order to try to connect to the accelerometers
    ...
    c = Mpu9250::readByte(MPU9250_ADDRESS, WHO_AM_I_MPU9250);  // Read WHO_AM_I register for MPU-9250
...
void Mpu9250::magcalMPU9250(float * dest1, float * dest2)
{
  ...
  for (ii = 0; ii < sample_count; ii++) {
    ...
    digitalWrite(myLed, !digitalRead(myLed));  // toggle led
    Serial.println("Magnetic Calib. " + String(ii) + "/" + String(sample_count));
  }
