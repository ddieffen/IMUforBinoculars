...
#define Ki 0.0f
  ...
  public:
    void Setup();
    void Compute(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz);
...
float Imu::Yaw() {
  ...
  /* déclinaison magnétique à Quimper = 1.71 */
  return yaw;
}
float Imu::Pitch() {
  float pitch;
...
void Imu::Compute(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz) {
  Now = micros();
