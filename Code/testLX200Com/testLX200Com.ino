...
void loop() {
  ...
  double lst = LocalSideralTime(uYYYY, uMM, uDD, uhh, umm, uss, lonE); //return local sideral time
  de = Declinaison(az, al, latN);
  ra = RightAscention(az, al, latN, de, lst);

  //Serial.println("AZ=" + String(az) + " Elev=" + al);

  double h = int(ra);
  double m = int((abs(ra) - abs(int(ra))) * 60);
  double s = (abs(ra) - abs(h)) * 3600 - (m * 60);
  //Serial.print("RA=" + Zeros(h) + ":" + Zeros(m) + ":" + Zeros(s) + "#");
  ...
  //Serial.println(" DEC=" + deg + "*" + Zeros(mm) + "#");
...
///
/// This function listens at the serial port and responds appropriately
/// If it sees the command "#:GR#" it responds with the Right Ascention
/// If it sees te command "#:GD#" it responds with the Declinaison
///
void communication()
{
  int i = 0;
